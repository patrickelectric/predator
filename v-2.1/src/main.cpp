...
void *image_show( void *)        /*analiza imagem*/
{
    ...
    while(1)
    {
        ...
        #else
            circle(frameCopy, Point( filterx.filter(alvo.x,timer_image_show.end()*3) ,filtery.filter(alvo.y,timer_image_show.end()*3)), 3, cvScalar(0,0,255), 1, 8, 0);
        #endif
        
        /// Make a simple text to debug
        char str[256];
        sprintf(str, "x:%d/y:%d", alvo.x, alvo.y);
        putText(frameCopy, str, cvPoint(filterx.filter(alvo.x,timer_image_show.end()*3)+30,filtery.filter(alvo.y,timer_image_show.end()*3)-30), FONT_HERSHEY_COMPLEX_SMALL, 0.5, cvScalar(0,0,255), 1, CV_AA);
